...
$query1 = "SELECT count(*) as total from posts A, members B where A.active='1' AND A.USERID=B.USERID AND A.phase='0' AND A.pic!='' AND A.nsfw='0' order by A.time_added desc limit $config[maximum_results]";
$query2 = "SELECT A.*, B.username from posts A, members B where A.active='1' AND A.USERID=B.USERID AND A.phase='0' AND A.pic!='' AND A.nsfw='0' order by A.time_added desc limit $pagingstart, $mobile_per_page";
