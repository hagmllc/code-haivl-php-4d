...
function insert_get_feat_gags($a)
{
    global $config,$conn;
	$query = "select A.PID, A.nsfw, A.story, A.pic, B.username from posts A, members B WHERE A.rec='1' AND A.active='1' AND A.youtube_key='' AND A.fod_key='' AND A.USERID=B.USERID order by rand() limit 5"; 
	$results = $conn->execute($query);
	$returnthis = $results->getrows();
	return $returnthis;
}
...
{
    $months = "";
    $allmonths = array("","Tháng 1","Tháng 2","Tháng 3","Tháng 4","Tháng 5","Tháng 6","Tháng 7","Tháng 8","Tháng 9","Tháng 10","Tháng 11","Tháng 12");
    for($i=1;$i<=12;$i++)
    {
        if($i == $selected)
		{
